...
public class InviaEmailController implements InviaEmailInterface {
	...
	public StreamedContent salvaInLocal(List<String> pdfScelti, Utente utenteLoggato,Boolean sceltaPdf) {
		...
		try {
			InputStream is=new ByteArrayInputStream(sendPdf.creaPdfAdmin(trovaListeScelte(pdfScelti) ,utenteLoggato,sceltaPdf));
			StreamedContent doc=DefaultStreamedContent.builder().contentType("application/pdf").name("C://Users//RobertVM//.eclipse//workspace2023//pdf/proviamo.pdf").stream(()->is).build();
			return doc;
